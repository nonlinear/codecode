...
gulp.task('start2', function(){
  return gulp.src(config.main.internal + 'start/index.html')
  .pipe(gulp.dest(config.main.src + 'pages/'))
});
gulp.task('start3', function(){
  return gulp.src(config.main.internal + 'start/layout.html')
  .pipe(gulp.dest(config.main.src + 'templates'))
});
gulp.task('start4', function(){
  return gulp.src(config.main.internal + 'start/layout.sass')
  .pipe(gulp.dest(config.main.src + 'sass'))
});
gulp.task('start5', function(){
  return gulp.src(config.main.internal + 'start/style.sass')
  .pipe(gulp.dest(config.main.src + 'sass'))
});
gulp.task('start6', function(callback){
  return gulp.src(config.main.internal + 'start/head.html')
